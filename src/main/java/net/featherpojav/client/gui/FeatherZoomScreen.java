...
public class FeatherZoomScreen extends Screen {
    private final Screen parent;
    private boolean listening = false;
    private ButtonWidget bindButton;

    public FeatherZoomScreen(Screen parent) {
    ...
    protected void init() {
        ...
        bindButton = ButtonWidget.builder(Text.of(getKeyNameText(FeatherPojavModClient.zoomKey)), button -> {
            listening = true;
    ...
    private void adjustMag(float val) {
        FeatherConfig.INSTANCE.zoomMagnification = Math.max(2.0f, Math.min(10.0f, FeatherConfig.INSTANCE.zoomMagnification + val));
        FeatherConfig.save();
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (listening) {
            ...
            if (keyCode != GLFW.GLFW_KEY_ESCAPE) {
                FeatherPojavModClient.zoomKey.setBoundKey(InputUtil.fromKeyCode(keyCode, scanCode));
                if (this.client != null && this.client.options != null) {
                    this.client.options.write();
                }
            }
            bindButton.setMessage(Text.of(getKeyNameText(FeatherPojavModClient.zoomKey)));
            return true;
        }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String desc = String.format("Current Magnification: %.1fx", FeatherConfig.INSTANCE.zoomMagnification);
